...
/**
 * Mobile card representation of a carrier My-Shipments-workspace row
 * (Milestone 4 Phase 12) — the `md:hidden` counterpart to the desktop
 * table. Renders only server-provided facts: `nextAction`
 * (shipmentNextAction) is shown verbatim, never re-derived from `status`
 * here. Links into the existing, unified `/marketplace/:id` Shipment
 * Detail — no separate route. Deliberately shares `ShipmentListItem` with
 * the shipper's `ShipmentCard` (same read model, same authoritative
 * facts) but is a distinct component since the two roles show different
 * counterpart facts (shipper name vs. carrier name).
 */
export function CarrierShipmentCard({ s }: { s: ShipmentListItem }) {
  return (
    <Link
